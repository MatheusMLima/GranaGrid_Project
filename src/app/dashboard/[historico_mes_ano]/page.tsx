...
const mesesHistoricoIndex: { [key: string]: number } = {
    'janeiro': 0,
    'fevereiro': 1,
    'março': 2,
    'abril': 3,
...
export default async function DashboardHistorico({ params }: { params: Promise<{ historico_mes_ano: string }> }) {
    const { historico_mes_ano } = await params
    ...
    const mesHistorico = mesesHistoricoIndex[historico_mes_ano.split("-")[0].toLowerCase()]
    const anoHistorico = Number(historico_mes_ano.split("-")[1])
    const dataAtual = new Date()

    if (dataAtual.getMonth() === mesHistorico && dataAtual.getFullYear() === anoHistorico) {
        redirect("/dashboard")
    }
    ...
    const dataNow = new Date(anoHistorico, mesHistorico, 1)
